...
text; daily hotspot counts for Sumatra, Kalimantan, Peninsular Malaysia, and
Sabah & Sarawak (regional fire activity isn't Indonesia-only, and counts are
cheap to capture fully); and NOAA-20 false-color satellite imagery for
Sumatra and Kalimantan specifically (the two regions matching haze-replay's
FIRMS bbox and Indonesia-source/Singapore-response narrative -- imagery is
comparatively expensive to store, so stays narrower than the counts). Built
to support the [haze-replay](https://github.com/impaural/haze-replay)
project's health banner and future citation needs.
...
data/asmc/
  YYYY-MM-DD/
    ...
    hotspot_counts.json   raw response, Sumatra/Kalimantan/P_Malaysia/SabahSarawak, past 7
                          days, day/high-confidence
